...
describe("GET /api/local/", () => {
    ...
    it("should handle missing temperature data gracefully", async () => {
        (fetch as jest.MockedFunction<typeof fetch>).mockResolvedValue(new Response(JSON.stringify([{ type: "humidity", value: 60, unit: "%" }]), { status: 200 }));
    ...
    it("should handle missing humidity data gracefully", async () => {
        (fetch as jest.MockedFunction<typeof fetch>).mockResolvedValue(new Response(JSON.stringify([{ type: "temperature", value: 22, unit: "C" }]), { status: 200 }));
